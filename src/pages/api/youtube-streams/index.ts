...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getYoutubeStreams();
    case 'POST':
      return createYoutubeStream();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getYoutubeStreams() {
    const data = await prisma.youtube_stream
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'youtube_stream'));
  ...
  async function createYoutubeStream() {
    ...
    const data = await prisma.youtube_stream.create({
      data: body,
    });
    return res.status(200).json(data);
  }
